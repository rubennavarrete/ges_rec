...
async function paginarDatosRecetas(page, size, modelo, columna, parametro) {
    const startIndex = (page - 1) * size;
    const endIndex = startIndex + size;
    ...
    filteredData.sort((a, b) => {
        if (a.bln_vigencia === b.bln_vigencia) {
            // Si los estados son iguales, ordenar por fecha de actualización de forma descendente
            return new Date(b.dt_fecha_creacion) - new Date(a.dt_fecha_creacion);
        } else {
            // Si los estados son diferentes, ordenar por estado de forma descendente
            return b.bln_vigencia - a.bln_vigencia;
        }
    });
...
export { paginarDatos, paginarDatosRecetas };